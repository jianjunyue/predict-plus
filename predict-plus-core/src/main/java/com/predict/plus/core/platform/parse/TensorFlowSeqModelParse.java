...
@Slf4j
@Component
public class TensorFlowSeqModelParse implements IModelParse {
    ...
    public BoosterAndFeatureConfigModel modelParse(InputStream inputStream, Integer version) {
        try {
            ...
            log.error("TensorFlowSeqModelParse-模型文件解析异常-version:{}-Exception", version, e);
        } finally {
            if (null != inputStream) {
                try {
                    inputStream.close();
        ...
        log.error("TensorFlowSeqModelParse-模型文件解析异常-version:{}", version);
        return null;
    }
